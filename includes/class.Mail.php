...
class Mail
{
   ...
   {
      $parts = explode('@', $email);
      return substr(base64_encode(md5(md5($parts[1].$login.$parts[0])).$pass), 0, CONFIRM_HASH_LENGTH);
   }
   ...
   public function send_mail($to, $subject, $text, $from, $fromName)
   {
      // $hash = $this->getUniqueSignature($email, $user->login, $user->password);
      
      if ($this->isLocalhost) {
         $this->saveToFile($to, $subject, $text);
      } else {
